...
function Generator(file, list, images, settings) {
    ...
    //console.log("+++++++++++++++++++++++++++++++");
    //console.log("x : ", list_['x']);
    //console.log("y : ", list_['y']);
    //console.log("z : ", list_['z']);
    //console.log("+++++++++++++++++++++++++++++++");
...
Generator.prototype = {
    ...
    after: function (image, arr_selector, direct, scale, list) {
        ...
        let imgOutPath = path.join.apply(this, [this.settings.imgRealOutputDir, this.settings.cssFileName + ext]);
        util2.write(imgOutPath, image.encode('png'));
        let cssImageRelativePath = path.relative(this.settings.cssRealOutputDir, this.settings.imgRealOutputDir);
        cssImageRelativePath = path.join.apply(this, [cssImageRelativePath, this.settings.cssFileName + ext]);
    ...
    zFill: function (list, scale) {
        ...
        let i, k, k0, length, images = [[], []], parsed = [[], []], max = [0, 0], total = [0, 0];
        ...
        if (images[left]) {
            ...
            for (i = 0, length = images[left].length; i < length; i++) {
                ...
                image.draw(Image(current.image), x, y);
                for (j = 0, count = current.cls.length; j < count; j++) {
                    let x_, y_ = (current.cls[j].position[1] + -y) + 'px';
                    if (scale) {
