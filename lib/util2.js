...
let _ = module.exports = function (path) {
...
function getFileTypeReg(type) {
    let map = [], ext = "";// fis.config.get('project.fileType.' + type);
    if (type === 'text') {
        map = TEXT_FILE_EXTS;
    } else if (type === 'image') {
        map = IMAGE_FILE_EXTS;
...
_.del = function (rPath, include, exclude) {
    ...
    if (rPath && _.exists(rPath)) {
        ...
        let isFile = stat.isFile() || stat.isSymbolicLink();
...
_.pipe = function (type, callback, def) {
    let processors = "";//fis.config.get('modules.' + type, def);
    if (processors) {
        let typeOf = typeof processors;
        if (typeOf === 'string') {
        ...
        processors.forEach(function (processor, index) {
            let typeOf = typeof processor, key;
            if (typeOf === 'string') {
                key = type + '.' + processor;
            ...
            if (typeof processor === 'function') {
                let settings = fis.config.get('settings.' + key, {});
...
_.download = function (url, callback, extract, opt) {
    ...
    let http = opt.protocol === 'https:' ? require('https') : require('http'),
        name = _.md5(url, 8) + _.ext(url).ext,
        tmp = fis.project.getTempPath('downloads', name),
        data = opt.data;
    delete opt.data;
    _.write(tmp, '');
    let writer = fs.createWriteStream(tmp),
...
_.install = function (name, version, opt) {
    ...
    let remote = opt.remote.replace(/^\/$/, '');
    let url = remote + '/' + name + '/' + version + '.tar';
    let extract = opt.extract || process.cwd();
    if (opt.before) {
    ...
    _.download(url, function (err) {
        if (err) {
            ...
            let pkg = _(extract, 'package.json');
            if (_.isFile(pkg)) {
                let info = _.readJSON(pkg);
                fs.unlinkSync(pkg);
